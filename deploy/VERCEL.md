...
The repo root now declares matching `next` / `react` versions and builds via `scripts/vercel-build-storefront.mjs`, which runs the storefront build and stages `.next` at the repo root for the Next.js builder.
